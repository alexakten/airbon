...
const StepCard: React.FC<StepCardProps> = ({
  ...
  const borderClass = showBorder ? "border max-w-none border-zinc-100 p-6 shadow-md text-left" : "text-center";
  const hoverClass = hoverEffect ? "hover:-translate-y-2" : "";


  return (
    <div
      className={`bg-white h-full max-w-xs ${borderClass} justify-between md:max-w-none md:text-left items-center md:items-start flex flex-col  w-full rounded-xl transform transition duration-300 ease-in-out ${hoverClass}`}
    >
