...
// Dynamically import the Map components, ensuring they're only loaded client-side
const MapContainerNoSSR = dynamic(() => import('react-leaflet').then(mod => mod.MapContainer), {
  ssr: false
});
const TileLayerNoSSR = dynamic(() => import('react-leaflet').then(mod => mod.TileLayer), {
  ssr: false
});
const MarkerNoSSR = dynamic(() => import('react-leaflet').then(mod => mod.Marker), {
  ssr: false
});
const PopupNoSSR = dynamic(() => import('react-leaflet').then(mod => mod.Popup), {
  ssr: false
});
...
const FarmerMap: React.FC<FarmerMapProps> = ({ farmers }) => {
  return (
    ...
    >
      ...
      {farmers.map((farmer) => (
        <MarkerNoSSR key={farmer.id} position={[farmer.latitude, farmer.longitude]}>
          <PopupNoSSR>
